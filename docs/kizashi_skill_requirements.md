...
- レポートには、本日のテーマ、シグナル、変化の兆し、周辺調査・比較、未確定・反証条件、過去レポート・既存知識との接続、source verification metadata、課題の深化、仮説の更新、次へのアイデア、編集後記、Feedback を含める
- 重要シグナルは、元ソースの記述だけで判断せず、関連URL、周辺記事、公式ドキュメント、過去レポート、既存wikiページ、反証候補を確認し、何が確認できたか・何が未確定かに加えて、source 間の差分、補完関係、矛盾、残る不確実性を残す
- X、Reddit、はてな、個人ブログ、まとめ記事、ブックマーク、二次紹介は原則 pain seed または social signal として扱い、重要シグナルに昇格する場合は元投稿URL、リンク先記事、公式情報、関連一次情報を可能な限り確認する
- 確認できない social source は `observed` または `unverified` として扱い、actionable claim にしない。ログイン、削除、非公開、アクセス制限などで確認できない場合は制約を明記する
...
Daily Report の依頼に対して、インラインの戦略要約、Review/Positioning レポート、独自レイアウトのHTML、Markdownの貼り付けだけで終わるのは未完了である。必ず日付入りの `.md` を作成し、HTMLを作る場合は標準テンプレートを使った `.html` も保存する。

日本語レポートでは、表示されるラベルと生成項目名を日本語にする。仮説ラベル・仮説名、シグナル番号・シグナル名、次へのアイデア名、表見出し、判断ラベルに `Signal`、`Updated Hypothesis`、`Idea`、`Why Matters`、`Change`、`Evidence`、`Decision`、`Source File`、`Strength`、`Novelty`、`internal note` などの英語UIラベルを残してはいけない。製品名、リポジトリ名、URL、ファイルパス、一次情報の正式タイトルは、追跡性を優先して原語のままでもよい。

日本語レポートでは、ラベルだけでなく本文の読みやすさも品質要件とする。英語の計画文を直訳したような硬い文章を避け、主張、根拠、含意が追いやすい自然な日本語にする。長すぎる文は分け、抽象語は具体的な場面、行為者、成果物、判断に接続する。
...
- `assets/templates/daily-signal-report.html` の `<style>` ブロックを保持する
- hero 背景画像 `https://hachiware-labs.com/images/hero/lab-building-with-sign.png` を保持する
- brand 画像 `https://hachiware-labs.com/images/hero/hachiware-character.png` を保持する
- `shell`、`hero`、`report-overview`、`signal-card`、`pain-point`、`context-note`、`feedback-note` などの主要クラスを保持する
- `{{...}}` プレースホルダーを残さない
- 日本語レポートでは、仮説名、シグナル名、次へのアイデア名、主要ラベルを日本語にする
- `scripts/validate-daily-report.js <report.html>` を通す
...
- 詳細な本文。通常は2段落以上で、状況、構造、なぜ重要かが分かること
- 変化の兆し
- 重要シグナルの場合は周辺調査の結果、確認できたこと、未確定事項、反証条件、source 間の差分、補完関係、矛盾、残る不確実性
- 過去レポート・既存知識との接続
- 1〜2個のペインポイント
- `ソースファイル`
- `元URL`
...
- 通常は3件以上のシグナル。ソースが少なく3件未満になる場合は理由を明記する
- 課題の深化は4項目程度
- 仮説の更新は2項目程度
- 次へのアイデアは3項目以上で、重要性、最小検証、成否条件、観測指標を短文で済ませない
- 編集後記は5段落以上、読み物として15〜20行程度の厚みを目安にする

編集品質:

- 各見出しを単なる必須セクションではなく、編集上の問いとして扱う
- シグナルは source 要約ではなく、「今日何が不可逆に変わりつつあるのか」から書く
- 課題の深化は抽象カテゴリではなく、現場で詰まる具体場面から始める
- 仮説の更新は作成/継続/絞り込み/強化/弱化/統合/保留などの判断差分を書く
- 次へのアイデアは artifact 名だけでなく、最小検証、成否条件、なぜ今やるかを書く
- 編集後記は全体の読み筋を回収し、次回観測する問いで終える
- source summary の羅列やチェックリスト充足だけの文章になっている場合は、一度だけ改稿する

日本語品質:

- 直訳調、機械翻訳調、硬すぎる計画文を避ける
- 1文に複数の主張を詰め込みすぎず、長い文は分ける
- 抽象語の連続を避け、場面、行為者、成果物、判断に接続する
- `ただし`、`一方で`、`そのため`、`ここで重要なのは`、`次に見るべきなのは` などの接続を自然に使う
- 助詞、同じ文末の連続、不自然なカタカナ語、名詞句の連結を見直す

意思決定ログとしての深さ:

- 重要シグナルでは source を単に列挙せず、公式 source 同士の差分、補完関係、矛盾、残る不確実性を書く
- 反証候補は「十分かもしれない」で止めず、どの条件なら仮説が弱まるか、または外部 artifact / ledger / skill / pipeline が不要になるかを書く
- 次へのアイデアは artifact 名ではなく、どの1 PR、どの1 skill candidate、どの1 UI artifact、どの1 source set、どの1 user task で試し、何を成功/失敗/指標として見るかを書く
- 深掘りに使える source がない場合は断定せず、次に確認する source / 条件として残す

Social / secondary source verification:

- X、Reddit、はてな、個人ブログ、まとめ記事、ブックマーク、二次紹介は pain seed または social signal として扱う
- 重要シグナルにする場合は、元投稿URL、リンク先記事、公式情報、関連一次情報を可能な限り確認する
- 投稿が参照している記事、リリース、docs がある場合は、投稿だけでなく参照先を確認する
- 確認できなかった制約を明記する
- 未確認の social source は `observed` または `unverified` に留め、actionable claim にしない
- Daily Report には、必要に応じて `確認済みソース`、`参照元ソース`、`検証ステータス`、`ソース階層`、`主張ステータス` を残す
- X / Reddit の単発投稿は proof として扱わず、公式 source、複数ユーザーの繰り返し、実例、issue、docs のいずれかで支えられるまで仮説の補助 evidence に留める

レビュー時のチェック:

- `<wiki>/reports/daily/` に保存され、`index.md` と `log.md` に反映されている
- HTML は `assets/templates/daily-signal-report.html` のデザインシェル、hero 背景画像、brand 画像、固定見出し、フィードバック欄を保持している
- `Trend Map` など独自見出しで Daily Report の流れを置き換えていない
- 日本語レポートでは、仮説名、シグナル名、次へのアイデア名、表見出し、判断ラベルが日本語になっている
- 各セクションが分量の下限を満たしている
- 各シグナルにソースファイルと元URLまたは内部メモが見える
- HTML は `scripts/validate-daily-report.js <report.html>` を通過している
- validator 合格だけで完了扱いにせず、読み筋、具体性、差分、判断可能性、文章密度の editorial review を行う
- editorial review が弱い場合は一度だけ改稿し、再度 validator を通す
- 日本語レポートでは Japanese readability review を行い、自然さ、文の長さ、具体語、接続、助詞や文末の表面品質を確認する
- 日本語が硬い、直訳調、抽象的すぎる、読み進めにくい場合は、根拠や構造を弱めずに文面だけを一度整える
- editorial review 後に decision-log depth review を行い、周辺調査の厚み、反証候補の具体性、次の検証の小ささを確認する
- decision-log depth が弱い場合は全文を改稿せず、重要シグナルの周辺調査、反証条件、次へのアイデア / 検証条件だけを一度増強し、再度 validator を通す
- social signal が重要シグナルになっている場合、元投稿またはリンク先、公式情報、関連一次情報を確認したかを確認する
- 二次情報を一次情報のように扱っていないか、確認できなかった制約が明記されているか、source tier / claim status が適切かを確認する
