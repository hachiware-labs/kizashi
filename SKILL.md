---
name: kizashi
description: Vercel Skills-compatible coding-agent skill for generating, refining, evaluating, and reviewing Hachiware Labs problem hypotheses from configured information sources such as X bookmarks, wiki-garden notes, release notes, GitHub issues, technical articles, and competitive signals. Use when Codex needs to initialize a Kizashi workspace, run daily or weekly hypothesis reviews, convert raw signals into testable pain hypotheses, update evaluations, create concise logs, or produce Kizashi reports.
---
...
9. Write the requested output in the user's locale: hypothesis, evaluation, daily log, validation plan, or periodic report.
...
```text
Use $kizashi to run weekly_kizashi_review end to end. Use `kizashi run` to prepare the source inventory, signals file, agent task, report seed, and log seed, then complete the semantic review: improve existing hypotheses first, create at most three new hypotheses only if pain, user count, or business scale spikes, update evaluations, and write the report in Japanese.
```
...
When the CLI is not globally installed, run the bundled scripts directly from the skill or repository, for example `node bin/kizashi.js run weekly_kizashi_review --target <project>`. Use `npm` only as an optional packaging, install, or test convenience.

## Modes

- **Init**: create the `kizashi/` workspace structure and seed config/templates. Use `node scripts/init-kizashi-project.js`.
- **Daily Log Mode**: process small daily inputs, update existing hypotheses lightly, and write a 10-30 line log under `kizashi/logs/YYYY/MM/YYYY-MM-DD.md`.
- **Periodic Review Mode**: synthesize weekly or monthly source changes, update evaluations, identify important hypotheses, and write `kizashi/reports/YYYY-MM-DD-kizashi-review.md`.
...
kizashi hypothesize --from kizashi/inputs/x-bookmarks/YYYY-MM-DD.md
...
- Treat Kizashi as source-driven. Name the source files used in every report or log.
...
- Keep daily logs short. Put analysis in hypotheses, evaluations, validations, or reports.
...
- `references/periodic-review.md`: weekly/monthly report workflow and report structure.
- `references/daily-log.md`: daily log rules and template.
...
- Run `kizashi run <task-id> --target <project>` as the top-level workflow command. It reads task/source config, inventories source inputs, extracts URL/heading/bullet signal candidates, writes `kizashi/signals/*.signals.md`, creates run `AGENT_TASK.md` and `manifest.json`, seeds report/log files, and tells the coding agent what semantic work remains.
...
- Run `node scripts/run-task.js <task-id> --target <project>` to execute the same top-level workflow command without a global CLI install.
