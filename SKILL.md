...
For Japanese Daily Reports, write all visible report text in Japanese by default. This includes report labels, generated prose, source metadata values, signal labels and titles, hypothesis labels and hypothesis names, idea names, table headers, metric labels, decision labels, helper labels, completion-gate notes, and automation summaries. Do not leave English UI labels, English status phrases, or agent-coined English noun phrases in the final visible report. Forbidden generated English includes labels/statuses such as `Signal`, `Updated Hypothesis`, `Idea`, `Why Matters`, `Change`, `Evidence`, `Decision`, `Source File`, `Strength`, `Novelty`, `Verified Source`, `Source Tier`, `Claim Status`, `actionable`, `supported`, `observed`, `official`, `social`, `Editorial self-review`, `Decision-log depth review`, and `Social / secondary source verification`, as well as non-proper generated phrases such as `runtime evidence`, `loop-readiness matrix`, `source-preserving edit loop`, `interface preservation`, `social claim promotion gate`, `reviewer`, `facilitator`, `adoption`, `proof`, `primary / official / social`, or `supported / observed`. Use Japanese equivalents such as `シグナル`, `更新仮説`, `アイデア`, `重要性`, `変更`, `根拠`, `判断`, `ソースファイル`, `強度`, `新規性`, `確認済みソース`, `ソース階層`, `主張ステータス`, `行動可能`, `根拠あり`, `観測`, `公式`, `社会的シグナル`, `編集レビュー`, `判断ログ深度レビュー`, `社会/二次ソース検証`, `実行時証拠`, `ループ準備度の比較表`, `根拠を保持する編集ループ`, `インターフェース保持`, `社会的シグナルの昇格ゲート`, `査読者`, `進行役`, `採用状況`, and `証拠`. Product names, repository names, URLs, file paths, official source titles, API/class names, code identifiers, and quoted source terminology may stay in their original language when translating them would reduce traceability, but surrounding sentences, generated concept names, item names, and status values must be Japanese. When in doubt, translate the phrase and keep the original English only in parentheses if traceability requires it.
...
- Language fidelity: generated hypothesis labels/names, signal labels/names, next idea names, table headers, decision labels, helper labels, source metadata values, and completion-gate notes must be in the user's language. For Japanese reports, reject visible labels, status phrases, and agent-coined English noun phrases such as `Signal`, `Updated Hypothesis`, `Idea`, `Why Matters`, `Change`, `Evidence`, `Decision`, `Source File`, `Strength`, `Novelty`, `Verified Source`, `Source Tier`, `Claim Status`, `actionable`, `supported`, `observed`, `official`, `social`, `Editorial self-review`, `Decision-log depth review`, `Social / secondary source verification`, `runtime evidence`, `loop-readiness matrix`, `source-preserving edit loop`, `interface preservation`, `social claim promotion gate`, `reviewer`, `facilitator`, `adoption`, `proof`, or `internal note` unless they are inside a URL, file path, code identifier, official product name, or quoted source title. Technical-product names may stay in English; generated explanation and generated item names should not.
...
- Daily report visible labels and generated item names must use the user's language. For Japanese reports, do not leave English labels or generated English noun phrases such as `Signal`, `Updated Hypothesis`, `Idea`, `Why Matters`, `Change`, `Evidence`, `Decision`, `runtime evidence`, `loop-readiness matrix`, `source-preserving edit loop`, or `social claim promotion gate` unless they are part of an official source title, product name, API/class name, code identifier, URL, or file path.
